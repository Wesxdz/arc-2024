# Alphabet Hash Bijection Mask

"""
For each ARC task, form two unique tile alphabets which share letters between tasks
The first with (1, n, m) for input matrices only
And the second (2, n, m) for IO tensors
Where n and m may be unique size permutations

Apply the following algorithm
For each n, m in range(l):
If an input matrices tile alphabet letter maps unambiguously (bijection) to an IO tensor tile alphabet letter input
Substitution is viable
Which is to say: if there is only one IO tensor tile alphabet letter with the same input matrix.
To determine this we simply create a hashmap from the input matrices alphabet letter keys to a list of IO tensor alphabet letters

Often, when there are multiple IO alphabet letters corresponding, we can generate a mask with the following procedure:
For each letter pair
    Are there any pixels within the letter whose corresponding output is the same for each io letter in value
We apply the substitution to the constant delta mask

Alhabima Beamiter is an iterative process (involving compound alphabet extractions)
"""
...
def abstract_array(input_array, tile_size, stride_length, starter_alphabet=None, modify_alphabet=True):
        ...
        for k in range(0, depth - tile_depth + 1, stride_depth):
            for i in range(0, rows - tile_rows + 1, stride_rows):
                for j in range(0, cols - tile_cols + 1, stride_cols):
                    tile = array[k:k+tile_depth, i:i+tile_rows, j:j+tile_cols]
                    tiles[(k, i, j)] = tile
        return tiles
    
    def create_alphabet(tiles, starter_alphabet=None):
        ...
        for (d, i, j), tile in tiles.items():
            ...
            if tile_key not in alphabet:
                alphabet[tile_key] = {'letter': letters[len(alphabet)], 'shape': tile.shape, 'tile': tile}
    ...
    # print(padded_array)
    tiles = extract_tiles(padded_array, tile_size, stride_length)
    if alphabet is None or modify_alphabet:
        alphabet = create_alphabet(tiles, starter_alphabet)
    abstracted_array = create_abstracted_array(padded_array, tiles, alphabet)
    
    return tiles, abstracted_array, alphabet
...
def modus_ponens(input_alphabet, input_matrix, abstracted_array, output_matrix, tile_size, P, batch_sub, bima_subs, delta_masks, alphabet_hash, fantasy_generator_map):
...
def cast_spell(scene):
    ...
    while True:
        ...
        input_tiles, input_abstracted_array, input_alphabet = abstract_array(input_pattern, tile_size, stride_length, input_alphabet)
        # print("Input Alphabet Abstracted Array:\n", input_abstracted_array)

        io_tiles, io_abstracted_array, io_alphabet = abstract_array(io_pattern, tile_size, stride_length, io_alphabet)
    ...
    solved_training = test_training_hypothesis(task_id, input_alphabet, tile_size, stride_length, alphabet_hash, delta_masks, fantasy_generator_map, False)
    
    if solved_training:
        test_outputs = propose_test_outputs(task_id, input_alphabet, tile_size, stride_length, alphabet_hash, delta_masks, fantasy_generator_map, False)
...
def test_training_hypothesis(task_id, input_alphabet, tile_size, stride_length, alphabet_hash, delta_masks, fantasy_generator_map, debug=False):
    i = 0
    ...
    while True:
        ...
        bima_subs = {}
        for v in last_n:
            modus_ponens(input_alphabet, input_pattern, input_abstracted_array, hypothesis_output, tile_size, v, batch_sub, bima_subs, delta_masks, alphabet_hash, fantasy_generator_map)
...
def test_io_regen(current_io, input_alphabet, tile_size, stride_length, alphabet_hash, delta_masks, fantasy_generator_map, debug=False):
    solved_all_training = True
    training_outputs = []
...
def propose_test_outputs(task_id, input_alphabet, tile_size, stride_length, alphabet_hash, delta_masks, fantasy_generator_map, debug=False):
    test_outputs = []
    i = 0
    while True:
        if debug: print(f"Solved with alphabet of size {len(input_alphabet)}")
        ...
        bima_subs = {}
        for v in last_n:
            modus_ponens(input_alphabet, input_pattern, input_abstracted_array, hypothesis_output, tile_size, v, batch_logic_sub, bima_subs, delta_masks, alphabet_hash, fantasy_generator_map)
...
if __name__ == "__main__":
    ...
    # Unsolved
    # scene = "b6afb2da" # almost! this one just needs to generalize a color equivalence rule (slate == null) to reduce alphabet
    # scene = "3aa6fb7a" # DONE! this one requires a fantasy map (rot n%90) to reduce alphabet
    # scene = "a5313dff"

    scene = "28e73c20"
    # Solved
    # these ones need (2, 2) tile shape to be solved...
    # scene = "0ca9ddb6"
    # scene = "4258a5f9"
